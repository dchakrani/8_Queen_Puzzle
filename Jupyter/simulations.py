...
def analyze_performance(problem_set, search_function):
    ...
    print('\n')
    print("Average Random Restart Required : " + str((restart + freq) / int(freq)) + '\n')
    #print ((restart + freq) / int(freq))
...
def analyze_all_algorithms(problem_set):
    ...
    analyze_performance(problem_set, lambda x, y: steepest_ascent_hill_climb(x, y, allow_sideways=True))
    print(section_break)

    print('Random restart hill climb:\n')
    analyze_performance(problem_set, lambda x, y: random_restart_hill_climb(problem_set[0].__class__, y))
    print(section_break)
    
    print('Random restart hill climb (up to 100 sideways moves allowed):\n')
    analyze_performance(problem_set, lambda x, y: random_restart_hill_climb(problem_set[0].__class__, y, allow_sideways=True))
    print(section_break)
...
queens_problem_set = [QueensProblem(n) for _ in  range(freq)]
